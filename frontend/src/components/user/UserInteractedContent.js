...
export class UserInteractedContent extends React.Component{
    ...
    GetAllPosts(){
        axios.get('http://localhost:8080/getlikedbyuser/'+this.state.userid).then((response)=>{
            const data = response.data;
            this.setState({likedposts:data});
        })
        .catch(()=>{alert('didnt retrieve liked posts')});
        axios.get('http://localhost:8080/getdislikedbyuser/'+this.state.userid).then((response)=>{
            const data = response.data;
